      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
        <div className="flex lg:flex-1">
          <Link href="/" className="-m-1.5 p-1.5">
            <span className="text-2xl font-heading font-bold text-[#2C5F7F] bg-white px-4 py-2 rounded-lg">
              LeaMatyi
            </span>
          </Link>
        </div>
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="text-sm font-semibold leading-6 text-[#2C5F7F] hover:text-[#1f4456] transition-colors"
            >
              {item.name}
            </Link>
          ))}
          ...
          <div className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            ...
            <div className="mt-6 flow-root">
              <div className="-my-6 divide-y divide-gray-500/10">
                ...
                <div className="py-6">
                  <Button className="w-full bg-[#2C5F7F] text-white hover:bg-[#1f4456] rounded-full">
                    Prihlásiť
                  </Button>
                </div>
              </div>
